...
//  $('#status').text("Microphone recorder event: " + arguments[0]);
  ...
  case "ready":
    ...
    Recorder.uploadFormId = "#recordForm";
    Recorder.uploadFieldName = "audio[sample]";
...
//    $('#save_button').css({'width': width, 'height': height});
  ...
  case "microphone_user_request":
    $("#flash_panel").attr('position', 'relative')
  ...
  case "microphone_connected":
    ...
    $("#flash_panel").css('position', 'absolute')
...
//    $('#record_button img').attr('src', 'images/stop.png');
    $('#record .record-button').toggleClass('hide')
    $('#record .send-button').addClass('disabled')
//    $('#play_button').hide();
    break;
...
//    $('#play_button').show();
    $('#record .record-button').toggleClass('hide');
    $('#record .send-button').removeClass('disabled');
    break;
...
//    $('#play_button img').attr('src', 'images/play.png');
  ...
  case "saved":
    ...
    ajaxSuccess(data);
//    if(data.saved) {
//      $('#upload_status').css({'colo=r': '#0F0'}).text(name + " was saved");
//    } else {
...
(function() {
  window.Recorder = {
    ...
    showPermissionWindow: function() {
      $("#flash_panel").css('position', 'static')
